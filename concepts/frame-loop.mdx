---
title: "The Fazen2d Frame Loop: Clear, Draw, Display"
sidebarTitle: "Frame Loop"
description: "Learn how Fazen2d's clear-draw-display cycle works, how the CHAR_INFO back-buffer is managed, and how to use delta time for frame-rate independent animation."
---
...
while (!game.keyboardHandler.CheckForUserExit()) {
    float dt = game.timer.GetElapsedTime();  // seconds since last call
    game.timer.Reset();
...
`GetElapsedTime()` uses `std::chrono::system_clock` under the hood. It measures the duration between the previous call and now, then advances its internal baseline. Calling `Reset()` after reading the value keeps the clock fresh for the next frame.

<Note>
The `TimeManager` constructor calls `Reset()` automatically, so the timer is ready from the moment `Fazen` is constructed. You only need to call `Reset()` manually in the loop body when you want to restart the measurement each frame.
</Note>
...
| `game.timer.Reset()` | Restart the internal `std::chrono` clock |
| `game.timer.GetElapsedTime()` | Return seconds elapsed since the last `Reset()` (or construction) |
